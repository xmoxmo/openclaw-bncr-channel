...
test('forces text payload attachments to file when mime major type is text', () => {
  assert.equal(resolveBncrOutboundMessageType({ hintedType: 'text', mimeType: 'text/javascript', hasPayload: true }), 'file');
});

test('falls back to file for unknown mime major type', () => {
  assert.equal(resolveBncrOutboundMessageType({ hintedType: 'unknown', mimeType: 'application/pdf', hasPayload: true }), 'file');
});
