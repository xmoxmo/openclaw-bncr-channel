...
test('parseRouteFromDisplayScope rejects old formats', () => {
  ...
  assert.equal(parseRouteFromDisplayScope('bncr:tgBot:10001'), null);
...
test('parseStrictBncrSessionKey parses route payload without forcing canonical main sessionKey', () => {
  const parsed = parseStrictBncrSessionKey('agent:main:bncr:direct:tgBot:0:10001');
  assert.ok(parsed);
  ...
  assert.equal(parsed.route.userId, '10001');
});
...
test('normalizeStoredSessionKey keeps strict and legacy sessionKey compatibility with canonicalAgentId override', () => {
  const strict = normalizeStoredSessionKey(
    'agent:main:bncr:direct:tgBot:0:10001',
    canonicalAgentId,
  );
  assert.ok(strict);
  ...
  const directLegacy = normalizeStoredSessionKey(
    `agent:main:bncr:direct:${Buffer.from('tgBot:0:10001', 'utf8').toString('hex')}:0`,
    canonicalAgentId,
  );
