...
test('deadLetter inspect and prune keep account isolation under deep pagination and olderThan filtering', async () => {
  const bridge = createBridge();

  try {
    const primaryA = makeEntry('dead-primary-a', 'a');
    primaryA.createdAt = 1_000;
    primaryA.lastError = 'push-retry';
    const primaryB = makeEntry('dead-primary-b', 'b');
    primaryB.createdAt = 5_000;
    primaryB.lastError = 'push-retry';
    const otherA = makeEntry('dead-other-a', 'c');
    otherA.accountId = 'Other';
    otherA.createdAt = 9_000;
    otherA.lastError = 'push-retry';
    bridge.deadLetter.push(primaryA, primaryB, otherA);

    const inspectCalls = [];
    await bridge.handleDeadLetterInspect({
      params: { accountId: 'Primary', reason: 'push-retry', olderThan: 6_000, limit: 1, offset: 1 },
      respond: (...args) => inspectCalls.push(args),
    });
    assert.equal(inspectCalls[0][1].total, 2);
    assert.equal(inspectCalls[0][1].entries.length, 1);
    ...
    assert.equal(pruneCalls[0][1].pruned, 2);
    assert.deepEqual(
      bridge.deadLetter.map((entry) => [entry.accountId, entry.messageId]),
      [['Other', 'dead-other-a']],
    );
  } finally {
    cleanupBridge(bridge);
