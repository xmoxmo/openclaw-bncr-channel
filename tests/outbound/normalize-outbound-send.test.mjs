...
function n(input) {
  return normalizeOutboundSend(input);
}

/* ===================================================================
 * 1. 文本化参数是否解析
 * =================================================================== */
test('[BncrParam] marker is parsed from text body', () => {
  const r = n({ text: 'hi [BncrParam:{"customKey":"val"}] there' });
...
test('[BncrParam] marker at start of text', () => {
  const r = n({ text: '[BncrParam:{"customKey":"val"}] hi there' });
  assert.equal(r.text, 'hi there');
  assert.equal(r.extra?.customKey, 'val');
});

test('[BncrParam] marker-only text with no other content', () => {
  const r = n({ text: '[BncrParam:{"customKey":"val"}]' });
  assert.equal(r.text, '');
  assert.equal(r.extra?.customKey, 'val');
});

test('[BncrParam] marker parsed from caption field', () => {
...
test('[BncrParam] no marker returns text unchanged', () => {
  const r = n({ text: 'plain hello world' });
  assert.equal(r.text, 'plain hello world');
  assert.equal(r.extra, undefined);
});

test('[BncrParam] invalid JSON marker stays in text for debugging', () => {
  const r = n({ text: 'bad [BncrParam:{invalid}] here' });
 ...
 * 2. 文本化参数是否在正文中剥离
 * =================================================================== */
test('[BncrParam] stripped from message body', () => {
  const r = n({ message: 'lead [BncrParam:{"k":"v"}] trail' });
  assert.equal(r.text, 'lead trail');
});

test('[BncrParam] stripped from caption body', () => {
  const r = n({ caption: 'cap [BncrParam:{"k":"v"}] end' });
  assert.equal(r.text, 'cap end');
});

test('[BncrParam] stripped from message when caption also present', () => {
...
// 3a. asVoice
test('[BncrParam] asVoice:true forces routing (no mediaUrl but hasMedia=true)', () => {
  const r = n({ text: '[BncrParam:{"asVoice":true,"path":"http://x/a.mp3","type":"audio"}]' });
  assert.equal(r.asVoice, true);
  assert.equal(r.hasMedia, true);
  assert.equal(r.mediaUrl, 'http://x/a.mp3');
});

test('[BncrParam] asVoice:false overrides host asVoice:true', () => {
  const r = n({
...
test('[BncrParam] type:file with remote path triggers media routing', () => {
  const r = n({
    text: '[BncrParam:{"type":"file","path":"http://cdn/doc.pdf"}] text',
  });
  assert.equal(r.hasMedia, true);
  assert.equal(r.type, 'file');
  assert.equal(r.mediaUrl, 'http://cdn/doc.pdf');
  // path stripped from extra
  assert.equal(r.extra?.path, undefined);
});

test('[BncrParam] type:voice promotes remote audio URL to media', () => {
  const r = n({
    text: '[BncrParam:{"type":"voice","path":"https://cdn/a.mp3"}] listen',
  });
  assert.equal(r.hasMedia, true);
  assert.equal(r.type, 'voice');
  assert.equal(r.mediaUrl, 'https://cdn/a.mp3');
  assert.equal(r.text, 'listen');
});

test('[BncrParam] type:appmsg with path is media evidence (default)', () => {
  const r = n({
    text: '[BncrParam:{"type":"appmsg","path":"http://img/thumb.jpg","msg":"<appmsg/>"}]',
  });
  // path is media evidence by default -> media branch
  assert.equal(r.hasMedia, true);
  assert.equal(r.mediaUrl, 'http://img/thumb.jpg');
...
test('[BncrParam] ismedia:true forces remote URL to media routing', () => {
  const r = n({
    text: '[BncrParam:{"ismedia":true,"path":"http://cdn/img.png"}]',
  });
  assert.equal(r.hasMedia, true);
  assert.equal(r.mediaUrl, 'http://cdn/img.png');
  // ismedia NOT in extra
  assert.equal(r.extra?.ismedia, undefined);
});

test('[BncrParam] ismedia:false does not force remote URL to media', () => {
  const r = n({
    text: '[BncrParam:{"ismedia":false,"path":"http://cdn/img.png"}] text',
  });
  // Not local, not media type, ismedia:false -> stays text
  assert.equal(r.hasMedia, false);
  assert.equal(r.extra?.path, 'http://cdn/img.png');
});

// 3d. downloadMedia tri-state
test('[BncrParam] downloadMedia:true overrides undefined', () => {
  const r = n({
    text: '[BncrParam:{"downloadMedia":true,"type":"image","path":"http://x/p.jpg"}]',
    mediaUrl: 'http://y/q.jpg',
  });
  assert.equal(r.downloadMedia, true);
  assert.equal(r.hasMedia, true);
});

test('[BncrParam] downloadMedia:false overrides host true', () => {
  const r = n({
    text: '[BncrParam:{"downloadMedia":false,"type":"image","path":"http://x/p.jpg"}] img',
    downloadMedia: true,
  });
...
test('[BncrParam] downloadMedia undefined when neither marker nor host set', () => {
  const r = n({ text: '[BncrParam:{"type":"image","path":"/tmp/a.jpg"}]' });
...
test('[BncrParam] consumed kind is applied and stripped from extra', () => {
  const r = n({ text: '[BncrParam:{"kind":"tool"}] hi' });
  assert.equal(r.kind, 'tool');
  assert.equal(r.extra?.kind, undefined);
});

test('[BncrParam] consumed replyToId is applied and stripped from extra', () => {
  const r = n({ text: '[BncrParam:{"replyToId":"123"}] hi' });
  assert.equal(r.replyToId, '123');
  assert.equal(r.extra?.replyToId, undefined);
});

// 3f. audioAsVoice consumed
test('[BncrParam] audioAsVoice consumed', () => {
  const r = n({
 ...
 * 4. 未知参数是否原样透传
 * =================================================================== */
test('[BncrParam] unknown params pass through to extra', () => {
  const r = n({
...
test('[BncrParam] multiple unknown params survive media routing (path stripped)', () => {
  const r = n({
    text: '[BncrParam:{"type":"image","path":"http://cdn/x.jpg","customField":"keep"}] img',
  });
  assert.equal(r.hasMedia, true);
...
test('[BncrParam] forceDocument and gifPlayback survive in extra', () => {
  const r = n({
 ...
 * =================================================================== */
test('[BncrParam] consumed fields stripped from extra (asVoice, type, ismedia)', () => {
  const r = n({
...
test('[BncrParam] media source fields stripped from extra (path, paths, mediaUrl, mediaUrls)', () => {
  const r = n({
    text: '[BncrParam:{"type":"image","path":"/tmp/a.jpg","paths":["/tmp/b.jpg"],"mediaUrl":"http://cdn/c.jpg","mediaUrls":["http://cdn/d.jpg"],"keepMe":"y"}]',
  });
  assert.equal(r.hasMedia, true);
  ...
  assert.ok(r.mediaUrls?.includes('/tmp/a.jpg'));
  assert.ok(r.mediaUrls?.includes('/tmp/b.jpg'));
...
test('[BncrParam] appmsg with explicit ismedia:false stays in text branch', () => {
  // Explicit ismedia:false overrides default path-as-media-evidence
  const r = n({
    text: '[BncrParam:{"ismedia":false,"type":"appmsg","msg":"<appmsg/>","path":"http://img/t.jpg"}]',
  });
  assert.equal(r.hasMedia, false);
  // type and path stay in extra for downstream scattering
  assert.equal(r.extra?.type, 'appmsg');
  assert.equal(r.extra?.msg, '<appmsg/>');
  assert.equal(r.extra?.path, 'http://img/t.jpg');
});

test('[BncrParam] forcedoc and host-level passthrough fields survive in extra', () => {
  const r = n({
    text: '[BncrParam:{"forceDocument":true,"gifPlayback":true,"silent":true}]',
 ...
 * =================================================================== */
test('[BncrParam] host-level asVoice and downloadMedia apply without marker', () => {
  const r = n({
    text: 'say hi',
    asVoice: true,
    downloadMedia: true,
...
test('[BncrParam] host downloadMedia:false passes through false (skip cascade)', () => {
...
test('[BncrParam] empty text and no media returns hasMedia=false, asVoice=false', () => {
  const r = n({});
  assert.equal(r.text, '');
  assert.equal(r.hasMedia, false);
  assert.equal(r.asVoice, false);
