...
function buildExplicitTargetResult(args: {
  raw: string;
  source:
    | 'display-scope'
    | 'strict-session-key'
...
function groupScopeToHex(route: BncrRoute): string {
  const raw = `${route.platform}:${route.groupId}`;
  return Buffer.from(raw, 'utf8').toString('hex').toLowerCase();
}

function directScopeToHex(route: BncrRoute): string {
...
function parseGroupRouteFromHexScope(scopeHex: string): BncrRoute | null {
  const rawHex = asString(scopeHex).trim();
  if (!isLowerHex(rawHex)) return null;
...
function parseDirectRouteFromHexScope(scopeHex: string): BncrRoute | null {
  const rawHex = asString(scopeHex).trim();
  if (!isLowerHex(rawHex)) return null;
...
function parseLegacySessionKey(input: string): {
  route: BncrRoute;
  inputKind: BncrSessionKind;
  inputAgentId?: string;
...
export function parseRouteLike(input: unknown): BncrRoute | null {
  ...
  const platform = asString(routeInput?.platform || '').trim();
  const groupId = asString(routeInput?.groupId || '').trim();
  const userId = asString(routeInput?.userId || '').trim();
  if (!platform || !groupId || !userId) return null;
  if (groupId !== '0' && userId !== '0') return null;
  return { platform, groupId, userId };
}
...
export function formatDisplayScope(route: BncrRoute): string {
  ...
  const groupId = asString(route?.groupId || '0').trim() || '0';
  const userId = asString(route?.userId || '0').trim() || '0';
...
function resolveExplicitTargetRoute(raw: string): {
